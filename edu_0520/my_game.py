...
def create_aliens():
    ...
    aliens = []
    for i in range(30):
        alien = pygame.rect.Rect(
            alien_x_pos, alien_y_pos, alien_rect.width, alien_rect.height
        )
        if i < 10:
            alien_x_pos += alien_rect.width * 2
            alien_y_pos += alien_rect.height
        elif 10 < i < 20:
            alien_x_pos += alien_rect.height
            alien_y_pos += alien_rect.height
        elif 20 < i < 30:
            alien_x_pos += alien_rect.height * 2
            alien_y_pos += alien_rect.height
        aliens.append(alien)
...
def create_bullet(ship_rect):
    global bullets
    bullet_rect = pygame.rect.Rect(0, 0, 5, 10)
